...
- 运行中**音量调节**：`F9`/`F10` 音量 -/+（0~1），功能键不影响打字
- **幽灵模式**：开启后找不到关闭的地方——无窗口、无控制台、无托盘图标、无任何提示、无退出快捷键
- 启动弹自定义 toast「已启动」；重复启动弹 toast「已在运行」并自动退出（tkinter 极简圆角卡片：底部居中、磨砂半透明、堆叠动画、跟随系统主题色，无系统弹窗）
...
pythonw main.py
```
启动成功弹自定义 toast「已启动，后台静默运行中」；若已有实例在运行，弹 toast「已在运行，请勿重复启动」并自动退出。toast 为 tkinter 极简圆角卡片（无系统弹窗，底部居中、磨砂半透明、堆叠动画、自动跟随系统深浅色与强调色）。
...
| 无界面提示 | 删除全部 `print`；仅启动/重复启动/切换时弹一次 toast |
| 启动反馈 | 自定义 tkinter toast：底部居中、圆角磨砂半透明、堆叠动画、跟随系统主题色（进程内绘制） |
...
- 调节键：`F10` 加、`F9` 减（功能键，不影响打字），步进 `ADJUST_STEP = 0.05`
...
| 按键 | 作用 | 范围 | 步进 |
|---|---|---|---|
| `F9` | 音量 -0.05 | 0 ~ 1.0 | 0.05 |
| `F10` | 音量 +0.05 | 0 ~ 1.0 | 0.05 |
...
| 音量调节无效 | 按键被其他程序占用 | 确认 F9/F10 未被占用；看 `keytone.log` |
